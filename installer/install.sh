...
docker run \
  --rm \
  --network composer_default \
  -v $(pwd)/rec-biznet.bna:/home/composer/rec-biznet.bna \
  -v $(pwd)/loc-stage:/home/composer/loc-stage \
  -v $(pwd)/.loc-card-store:/home/composer/.composer \
  hyperledger/composer-cli:latest \
  network install -c PeerAdmin@hlfv1 -a rec-biznet.bna
...
docker run \
  ...
  -v $(pwd)/.loc-card-store:/home/composer/.composer \
  hyperledger/composer-cli:latest \
  network start -n rec-biznet -V $NETWORK_VERSION -c PeerAdmin@hlfv1 -A admin -S adminpw -f /home/composer/loc-stage/bnaadmin.card
...
#   -v $(pwd)/.loc-card-store:/home/composer/.composer \
#   hyperledger/composer-cli:latest \
#   transaction submit -c admin@rec-biznet -d '{"$class": "org.rec.loc.CreateDemoParticipants"}'

# SET CORRECT PERMISSIONS
printHeader "SET CORRECT PERMISSIONS"
docker exec \
  composer \
  find /home/composer/.composer -name "*" -exec chmod 777 {} \;

# START THE REST SERVER
printHeader "START THE REST SERVER"
docker run \
  -d \
  --network composer_default \
  --name rest \
  -v $(pwd)/.loc-card-store:/home/composer/.composer \
  -e COMPOSER_CARD=admin@rec-biznet \
  -e COMPOSER_NAMESPACES=never \
  -p 3000:3000 \
  hyperledger/composer-rest-server:latest

#WAIT FOR REST SERVER TO WAKE UP
echo "WAIT FOR REST SERVER TO WAKE UP"
sleep 10
...
URLS="http://localhost:8080 http://localhost:3000/explorer/"
case "$(uname)" in
"Darwin") open ${URLS}
          ;;
"Linux")  if [ -n "$BROWSER" ] ; then
	       	        $BROWSER http://localhost:8080 http://localhost:3000/explorer/
                  ...
                  for URL in ${URLS} ; do
	                ...
	                gnome-open http://localhost:8080 http://localhost:3000/explorer/
	        else
